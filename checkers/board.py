...
class Board:
	...
	def get_valid_moves(self, piece):
		...
		if piece.color == WHITE or piece.king:
			moves.update(self.traverse_left(row + 1, min(ROWS, row + 3), 1, piece.color, left))
			moves.update(self.traverse_right(row + 1, min(ROWS, row + 3), 1, piece.color, right))
		if piece.color == BLACK or piece.king:
			moves.update(self.traverse_left(row - 1, max(-1, row - 3), -1, piece.color, left))
			moves.update(self.traverse_right(row - 1, max(-1, row - 3), -1, piece.color, right))
